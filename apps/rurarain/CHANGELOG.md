# CHANGELOG - RuraRain (formerly PlanejaChuva)
...
## Phase RAIN-10.1: CORE-90 Integration — FarmRole Adapter Registration
...
### Problem
Currently, when a user first opens the app, the default property is automatically created with the name "Minha Propriedade". This is generic and not meaningful when viewing weather data or statistics. Users should be prompted to enter their property/farm name for a more personalized experience.
...
### UX Flow
```
Consent Screen → "Accept" → Location Prompt → Property Name Prompt → Home Screen
                                              (if name is "Minha Propriedade")
```
...
## Phase RAIN-01: Rebrand PlanejaChuva → RuraRain

### Status: [DONE]
**Date Completed**: 2026-01-25
**Priority**: 🟡 ARCHITECTURAL
**Objective**: Rebrand app from "PlanejaChuva" to "RuraRain" for internationalization. Migrate to new Firebase project (RuraCamp).
**Cross-Reference**: CORE-70

### Changes Summary

1. **Folder & Package Rename**
   - Renamed folder `apps/planejachuva` → `apps/rurarain`
   - Updated `pubspec.yaml` name from `planejachuva` to `rurarain`

2. **Android Configuration**
   - Changed `namespace` from `br.com.planejacampo.planejachuva` to `com.ruracamp.rain`
   - Changed `applicationId` from `br.com.planejacampo.planejachuva` to `com.ruracamp.rain`
   - Updated flavor `app_name` from "PlanejaChuva" to "RuraRain"
   - Moved `MainActivity.kt` to new package structure `com/ruracamp/rain/`
   - Moved `RainWidgetProvider.kt` to `com/ruracamp/rain/widget/`
   - Created flavor directories with `google-services.json` for dev and prod

3. **iOS Configuration**
   - Changed `PRODUCT_BUNDLE_IDENTIFIER` from `br.com.planejacampo.planejaChuva` to `com.ruracamp.rain`

4. **Dart Code Updates**
   - Renamed `PlanejaChuvaApp` class to `RuraRainApp`
   - Updated all `appName` strings from "PlanejaChuva" to "RuraRain"
   - Updated all asset paths from `planejachuva-icon` to `rurarain-icon`
   - Updated `BackupProvider` key from `planeja_chuva` to `rura_rain`
   - Updated notification titles and backup subject lines

5. **Firebase Configuration**
   - Configured project `ruracamp-dev` for development
   - Configured project `ruracamp-c1f38` for production
   - Added SHA-256 fingerprint to both projects
   - Generated `firebase_options.dart` for dev environment

6. **Asset Rename**
   - Renamed icon files from `planejachuva-icon.*` to `rurarain-icon.*`

### Files Modified

| File | Action | Description |
|------|--------|-------------|
| `pubspec.yaml` | MODIFY | Changed name and description |
| `android/app/build.gradle` | MODIFY | Updated namespace, applicationId, flavor names |
| `android/app/src/main/AndroidManifest.xml` | MODIFY | Updated AdMob comment |
| `android/app/src/main/kotlin/com/ruracamp/rain/MainActivity.kt` | CREATE | New package location |
| `android/app/src/main/kotlin/com/ruracamp/rain/widget/RainWidgetProvider.kt` | CREATE | New package location |
| `android/app/src/main/kotlin/br/...` | DELETE | Old package structure |
| `android/app/src/dev/google-services.json` | CREATE | Dev Firebase config |
| `android/app/src/prod/google-services.json` | CREATE | Prod Firebase config |
| `ios/Runner.xcodeproj/project.pbxproj` | MODIFY | Updated bundle identifiers |
| `lib/main.dart` | MODIFY | Renamed app class, updated strings |
| `lib/screens/lista_chuvas_screen.dart` | MODIFY | Updated branding strings and icon paths |
| `lib/screens/configuracoes_screen.dart` | MODIFY | Updated icon paths |
| `lib/services/backup_service.dart` | MODIFY | Updated app name strings |
| `lib/services/chuva_backup_provider.dart` | MODIFY | Changed backup key |
| `lib/services/export_service.dart` | MODIFY | Updated app name constant |
| `lib/services/notification_service.dart` | MODIFY | Updated notification title |
| `lib/firebase_options.dart` | REGENERATE | New Firebase configuration |
| `assets/images/rurarain-icon.png` | RENAME | From planejachuva-icon.png |
| `test/widget_test.dart` | MODIFY | Updated import and class name |
| `CHANGELOG.md` | MODIFY | Updated header and prefix |
| `README.md` | MODIFY | Updated branding |
| `ARCHITECTURE.md` | MODIFY | Updated references |
