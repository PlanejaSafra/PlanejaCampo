...
class _HomeScreenState extends State<HomeScreen> {
  ...
  final ContaPagarService _contaPagarService = ContaPagarService(); // Serviço de ContaPagar
  ...
  void _initializeState() async {
    ...
    if (mounted) {
      setState(() {
        _showTutorial = appStateManager.showTutorial('homeScreen');
        _isInitialized = true;
  ...
  Future<Map<String, double>> _getTotaisMes() async {
    if (AppStateManager().hasActiveProdutor) {
      ...
      //print('Antes de entrar no for de pagamentos -----------------------------------');
      //print('Saiu do for de pagamentos -----------------------------------');

      double totalAPagar = contasPagar.fold(0.0, (sum, contaPagar) => sum + contaPagar.valor);
      //print('homeScreen: primeiro dia: $primeiroDia, ultimo dia: $ultimoDia, totalComprado: $totalComprado, total a pagar: $totalAPagar');

      return {'comprado': totalComprado, 'aPagar': totalAPagar};
    }
    return {'comprado': 0.0, 'aPagar': 0.0};
  }
  ...
  void _onTutorialFinished() {
    final AppStateManager appStateManager = Provider.of<AppStateManager>(context, listen: false);
    appStateManager.setShowTutorial('homeScreen', false);
    if (mounted) {
      setState(() {
        _showTutorial = false;
      });
